...
def assert_json_content_format_valid(path: Path) -> None:
    try:
...
def save_file_valid(path_str: str) -> Path:
    path = Path(path_str)
...
def load_file_valid(path_str: str) -> Path:
    path = Path(path_str)
    ...
    assert_json_content_format_valid(path)

    return path


def add_arg_load(parser: ArgumentParser) -> None:
    load_help_desc: str = ("file from which the model will be initialised ("
                           "Path)")

    parser.add_argument("--load", type=load_file_valid,
...
def add_arg_step_by_step(parser: ArgumentParser) -> None:
...
def add_arg_learning(parser: ArgumentParser) -> None:
    help_desc: str = "if the model should learn from each session"

    parser.add_argument("-l", "--learning", choices=["on", "off"],
                        default="off", help=help_desc)


def add_arg_visual(parser: ArgumentParser) -> None:
    help_desc: str = "visual mode on means a window will render the game state"

    parser.add_argument("-v", "--visual", choices=["on", "off"],
                        required=True, help=help_desc)


def add_arg_session(parser: ArgumentParser) -> None:
...
def init_parser() -> ArgumentParser:
    ...
    add_arg_session(parser)
    add_arg_visual(parser)
    add_arg_save(parser)
    add_arg_load(parser)
    add_arg_learning(parser)
    add_arg_step_by_step(parser)
